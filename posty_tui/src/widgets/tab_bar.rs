...
const MAX_TAB_TITLE_LENGTH: usize = 10;
...
///Block for render paths, code duplication but its better than littering the code with a crap ton
///of if let Some() statements.
impl TabBar {
    ///No themes are set for the component so it renders styled.  
    fn render_styled(&mut self, area: ratatui::prelude::Rect, buf: &mut ratatui::prelude::Buffer) {
        if area.height == 0 || area.width == 0 {
            return;
        }
        let y = area.top();
        let t = &self.config.theme.clone().unwrap();
        ...
        self.ensure_active_visible(area.width);
        let has_left_overflow = self.scroll > 0;
        let left_reserve: u16 = if has_left_overflow { 2 } else { 0 };
        let avail_right = area.width.saturating_sub(left_reserve);
        let visible = self.count_visible(avail_right);
        let has_right_overflow = self.scroll + visible < self.tabs.len();
        ...
        self.ensure_active_visible(tab_area_width);
        let visible = self.count_visible(tab_area_width);
        let has_left_overflow = self.scroll > 0;
        let has_right_overflow = self.scroll + visible < self.tabs.len();
        let mut x = area.left();
        ...
        for (vis_i, abs_i) in (self.scroll..self.scroll + visible).enumerate() {
            let tab = &self.tabs[abs_i];
            let is_active = abs_i == self.active;
    ...
    fn render_def(&mut self, area: ratatui::prelude::Rect, buf: &mut ratatui::prelude::Buffer) {
        ...
        // Clear the row with default style
        for x in area.left()..area.right() {
            buf.get_mut(x, y)
                .set_symbol(" ")
        ...
        for (vis_i, abs_i) in (self.scroll..self.scroll + visible).enumerate() {
            ...
            // Active tab gets BOLD to give it extra visual weight alongside the brackets
            let base_style = if is_active {
            ...
            // Opening bracket for active tab, plain space for inactive
            if x < right_tab_boundary {
                let sym = if is_active { "[" } else { " " };
                buf.get_mut(x, y).set_symbol(sym).set_style(base_style);
                x += 1;
            }
            ...
            if x < right_tab_boundary {
                let symbol = if tab.modified { "●" } else { "×" };
                buf.get_mut(x, y).set_symbol(symbol).set_style(base_style);
                x += 1;
            }

            // Closing bracket for active tab, plain space for inactive
            if x < right_tab_boundary {
                let sym = if is_active { "]" } else { " " };
                buf.get_mut(x, y).set_symbol(sym).set_style(base_style);
                x += 1;
...
impl Widget for &mut TabBar {
    ...
    where
        Self: Sized,
    {
        if self.config.theme.is_some() {
            self.render_styled(area, buf);
        } else {
            self.render_def(area, buf);
        }
    }
}
